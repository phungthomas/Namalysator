...
struct alto_rectangle
{
	...
	void resize_to_minimum_1()
	// Sometimes a dimension becomes 0 because of size reductions in the blockcutter
	// To prevent this we add this function which gives each dimension a minimum of 1 pixel
	{
		if (x < 1) { x = 1; }
		if (y < 1) { y = 1; }
		if (width < 1) { width = 1; }
