...
#ifndef NO_ERR_COLLECTOR
	//	g_err.add_message(err_major, module, "Cannot open" + std::string(name), true);
#endif
		return ((TIFF *)0);
	}
...
TIFFOpenW(const wchar_t* name, const char* mode)
{
	...
	if (fd == INVALID_HANDLE_VALUE) {
#ifndef NO_ERR_COLLECTOR
	//	g_err.add_message(err_major, module, "Cannot open file", true);
#endif
		return ((TIFF *)0);
	}
	...
	if (mbsize > 0) {
		...
		if (!mbname) {
#ifndef NO_ERR_COLLECTOR
	//		g_err.add_message(err_major, module, "Can't allocate space for filename conversion buffer", true);
#endif
			return ((TIFF*)0);
		}
